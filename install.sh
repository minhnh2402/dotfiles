...
mod_core() {
    ...
    # libclang-dev: needed by Rust bindgen crates and by
    # `cargo install tree-sitter-cli` if ever built from source.
    apt_install \
        build-essential make cmake ninja-build ccache gcc g++ gdb libclang-dev \
...
mod_cli() {
    ...
    # NOTE: fzf is intentionally NOT installed here.
    # apt on 22.04 ships fzf 0.29, but fzf-lua requires > 0.36.
    # fzf is installed from git in the `nvimtools` module instead.
    apt_install \
        htop btop tree ncdu \
        jq bat ripgrep fd-find tealdeer \
        strace lsof \
...
mod_zsh() {
    ...
    # NOTE: zsh is installed via apt. Unlike Neovim, the zsh project does
    # not publish prebuilt binaries to download; the only alternatives are
    # apt (5.8.x) or building from source (5.9). The version gap is tiny,
    # so apt is used here.
...
mod_neovim() {
    step "[neovim] Install Neovim (prebuilt release binary)"
 
    # Pinned version — bump this string to upgrade later.
    local NVIM_VERSION="v0.12.5"
    local NVIM_DIR="nvim-linux-x86_64"
    local NVIM_URL="https://github.com/neovim/neovim/releases/download/${NVIM_VERSION}/${NVIM_DIR}.tar.gz"
 
    # apt on 22.04 ships a Neovim far too old for native treesitter / fzf-lua.
    # Download the official prebuilt tarball instead of building from source.
    if [ "$(nvim --version 2>/dev/null | head -1)" = "NVIM ${NVIM_VERSION}" ]; then
        warn "Neovim ${NVIM_VERSION} already installed."
    else
        curl -fL "$NVIM_URL" -o "/tmp/${NVIM_DIR}.tar.gz"
        sudo rm -rf "/opt/${NVIM_DIR}"
        sudo tar -C /opt -xzf "/tmp/${NVIM_DIR}.tar.gz"
        rm -f "/tmp/${NVIM_DIR}.tar.gz"
        # Symlink into /usr/local/bin so it shadows any apt nvim, without
        # editing .zshrc (which is managed by stow).
        sudo ln -sf "/opt/${NVIM_DIR}/bin/nvim" /usr/local/bin/nvim
    fi
 
    step "Neovim: $(nvim --version | head -1)"
}
...
mod_nvimtools() {
    ...
    step "nvimtools done — fzf $(fzf --version | awk '{print $1}'), $(tree-sitter --version)"
}
...
declare -A MOD_DESC=(
    ...
    [cli]="Terminal utils (ripgrep, bat, fd, xclip, tealdeer...)"
    [zsh]="Zsh + Oh My Zsh + plugins + Powerlevel10k"
    [neovim]="Neovim v0.12.5 (prebuilt binary, not apt)"
    [nvimtools]="fzf (>0.36) + tree-sitter CLI for Neovim (apt too old)"
